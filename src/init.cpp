...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: XYLO.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: XYLOd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeXYLOamount=<n> " + _("Keep N XYLO anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. XYLO is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. XYLO is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("XYLO version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "XYLO server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of XYLO") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart XYLO to complete") << "\n";
    ...
    nAnonymizeXYLOAmount = GetArg("-anonymizeXYLOamount", 0);
    if(nAnonymizeXYLOAmount > 999999) nAnonymizeXYLOAmount = 999999;
    if(nAnonymizeXYLOAmount < 2) nAnonymizeXYLOAmount = 2;
    ...
    LogPrintf("Anonymize XYLO Amount %d\n", nAnonymizeXYLOAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1XYLO+1000 == (.1XYLO+100)*10
       10XYLO+10000 == (1XYLO+1000)*10
    */
