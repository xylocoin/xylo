...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeXYLOAmount"))
        settings.setValue("nAnonymizeXYLOAmount", 1000);
    nAnonymizeXYLOAmount = settings.value("nAnonymizeXYLOAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeXYLOAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeXYLOAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeXYLOAmount:
            return QVariant(nAnonymizeXYLOAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeXYLOAmount:
            nAnonymizeXYLOAmount = value.toInt();
            settings.setValue("nAnonymizeXYLOAmount", nAnonymizeXYLOAmount);
            emit AnonymizeXYLOAmountChanged(nAnonymizeXYLOAmount);
            break;
        default:
            break;
        }
