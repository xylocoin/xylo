...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        // they tried to consolidate 10 1k coins into one 100k coin, and ended up with 1k in change
        empty_wallet();
        for (int i = 0; i < 20; i++)
            add_coin(1000 * COIN);

        BOOST_CHECK( wallet.SelectCoinsMinConf(10000 * COIN, 1, 1, vCoins, setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 10000 * COIN); // we should get the exact amount
